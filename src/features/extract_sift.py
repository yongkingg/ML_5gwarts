...
def extract_sift_features():
    ...
    # SIFT 피처를 평균내어 1개의 벡터로 만듭니다 (Averaged SIFT)
    feature_name = "sift_avg"
    ...
    try:
        ...
        print(f" ! 에러: SIFT 객체 생성 실패. {e}")
        print(" ! OpenCV Contrib 모듈이 필요할 수 있습니다.")
        print(" ! 터미널에서 'pip install opencv-contrib-python'을 시도해 보세요.")
        return

    print(f"--- SIFT (Averaged) 피처 추출 시작 ---")
    ...
    for zone_name in sorted(os.listdir(processed_dir)):
        ...
        for img_name in tqdm(os.listdir(zone_path), desc=zone_name):
            ...
            try:
                # --- 3. SIFT 피처 추출 및 평균 집계 ---
                img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
                
                # 키포인트와 디스크립터(피처) 검출
                # 'des'는 (N, 128) 형태의 배열 (N=검출된 키포인트 수)
                kp, des = sift.detectAndCompute(img, None)
                
                if des is not None and len(des) > 0:
                    # N개의 128차원 피처를 평균내어 1개의 128차원 벡터로 만듦
                    feature_vector = np.mean(des, axis=0)
                else:
                    # SIFT 피처가 검출되지 않으면 128차원 0 벡터로 채움
                    feature_vector = np.zeros(128)
                
                # --- 모니터링 ---
                if MONITORING_ENABLED:
                    print(f"  > [모니터링] {img_name} SIFT Keypoints 시각화...")
                    # SIFT 키포인트를 원본 이미지(컬러)에 다시 그림
                    ...
                    plt.title(f'SIFT Keypoints: {img_name} ({len(kp)} points found)')
                    plt.axis('off')
                    plt.show()
                # --- 모니터링 끝 ---
